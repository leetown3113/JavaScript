...
const DEFAULT_OPTIONS = {
  title: 'Netflix Unlock',
  fullContent: '完整支持 ➤ #REGION_NAME#(#REGION_NAME_EN#)',
  fullIcon: 'checkmark.circle',
  fullIconColor: '#555555',
  fullStyle: 'good',
  onlyOriginalContent: '仅自制剧 ➤ #REGION_NAME#(#REGION_NAME_EN#)',
  onlyOriginalIcon: 'ellipsis.circle',
  onlyOriginalIconColor: '#555555',
  onlyOriginalStyle: 'info',
  notAvailableContent: '节点不支持Netflix',
  notAvailableIcon: 'exclamationmark.circle',
  notAvailableIconColor: '#555555',
  notAvailableStyle: 'alert',
  errorContent: '检测失败 请更换节点',
  errorIcon: 'multiply.circle',
  errorIconColor: '#555555',
  errorStyle: 'error',
  timeout: 6000,
}
...
;(async () => {
  await Promise.race([test(FILM_ID), timeout(6000)])
    .then(async region => {
      if (options.fullIcon) {
    ...
    .catch(async error => {
      ...
      let region = await Promise.race([test(AREA_TEST_FILM_ID), timeout(6000)])
