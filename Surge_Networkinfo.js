  ...
  if ($network['cellular-data']) {
    ...
    if ($network.wifi?.ssid == null && radio) {
      cellularInfo = carrierNames[carrierId] ?
        ...
        `ChinaTelecom | ${radioGeneration[radio]} - ${radio}`;
    }
  }
  return cellularInfo;
}
...
function getIP() {
  ...
  if (!v4 && !v6) {
    ...
    /*if (v6?.primaryAddress) info.push(`IPv6地址：It has been deployed`);*/
    if (v4?.primaryRouter && getSSID()) info.push(`路由IP：${v4?.primaryRouter}`);
    /*if (v6?.primaryRouter && getSSID()) info.push(`IPv6地址：It has been deployed`);*/
  }
  info = info.join("\n");
  return info + "\n";
}
