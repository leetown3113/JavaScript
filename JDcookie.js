 ...
 * @updated    2025-11-13
 * @version    v0.0.1
 * @link       https://raw.githubusercontent.com/cunzhiceyuan/ios-QX-re/refs/heads/main/surge/JD/JDcookie.js
 * ❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖❖
 * 主要功能：
 * 🔵 自动抓取京东 Cookie（pt_key + pt_pin）
 * 🔵 自动写入 BoxJS → CookiesJD
 * 🔵 自动识别该账号 Cookie  → 自动提交到 API：  
 *       http://rrror.top:9988/api/jd/submit_ck
 ...
 * a-JD_pt_key = type=http-request, pattern=^https?:\/\/api\.m\.jd\.com\/client\.action\?functionId=(wareBusiness|serverConfig|basicConfig), script-path=https://cf-row.newbingzzroy.workers.dev/https://raw.githubusercontent.com/cunzhiceyuan/ios-QX-re/refs/heads/main/surge/JD/JDcookie.js
 *
 * [MITM]
 ...
 * ------------------------------------------
 * [rewrite_local]
 * ^https?:\/\/api\.m\.jd\.com\/client\.action\?functionId=(wareBusiness|serverConfig|basicConfig) url script-request-header https://raw.githubusercontent.com/cunzhiceyuan/ios-QX-re/refs/heads/main/surge/JD/JDcookie.js
 *
 * [mitm]
 ...
 * ------------------------------------------
 * [Script]
 * http-request ^https?:\/\/api\.m\.jd\.com\/client\.action\?functionId=(wareBusiness|serverConfig|basicConfig) script-path=https://raw.githubusercontent.com/cunzhiceyuan/ios-QX-re/refs/heads/main/surge/JD/JDcookie.js, timeout=10, tag=京东Cookie获取
 *
 * [MITM]
 * hostname = api.m.jd.com
 *
...
function notifyResult(pt_pin, success, message, data) {
    ...
    if (success && data) {
        body += `\n\n昵称: ${data.jdname || pt_pin}`;
        body += `\n状态: ${data.is_new ? '新增' : '更新'}`;
       // body += `\n青龙同步: ${data.ql_sync ? '成功' : '失败'}`;
    }
